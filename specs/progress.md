...
| 13.1 | **Done** | 3/3 | 3/3 | — | **Post-13 fixes (`96d0c19`..`9982dd7`).** (a) DB ops: rebuilt empty `tracker.db` (had **no tables** → every endpoint 500'd); (b) `PATCH /api/action-items/{id}` (api-designed) + `due_date` passthrough on AI-Lead endpoint → AI-Lead status/due_date now **persist** (optimistic+rollback), overdue keys off **real `due_date`** not meeting_date; (c) clean **empty/error states across ALL pages** (shared `EmptyState`/`ErrorState`, no raw `ApiError: GET → 500`). Gate (2 consultants) + review = no FIX-NOW; simplify = no churn. Build green, PATCH 200/404 + passthrough + 45/45 journal verified on throwaway DB; tracker.db untouched. **⚠ Omer: restart uvicorn — running process was on stale pre-Wave-12 code (404 on ai-lead route, 500 on `/tasks`).** Deferred NITs: removed-id(404)→generic error not "not found"; swallowed-catch has no console.error; DomainPage/TeamPage loaders lack cancel-guard (pre-existing) |
