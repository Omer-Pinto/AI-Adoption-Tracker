...
> Fold the champion INTO the team (`team.champion_name NOT NULL` + `champion_start_date`; drop the `champion` table); key everything by `team_id`; team page `/teams/:teamId`; remove the report champion-picker; **nuke** dead `cc_baseline`/`baseline_date`; recreate the DB clean; fix the QA dataset (Web-Experience 2→1 champion). **Design approved** — ONE wave, 6 agents on disjoint files build to the target shape in parallel, cherry-picked together. No gate. 5 decisions locked (champion_name NOT NULL · nuke cc_baseline · team-chooser fallback · recreate clean · team+champion created together). See `task_breakdown.md` Wave 16.
...
> **Built + verified.** 17A/17B cherry-picked clean (`f3be3ee`,`1253978`, disjoint backend/FE). Same-type consultants audited both; FE↔BE contract reconciled (2 FIX-NOW: reset-password returns User + `new_password` optional; dropped `password` from UserUpdateBody). code-review caught a real merge bug — session was FK'd to `user(username)` so renaming a logged-in user 500'd → **re-keyed sessions to `user.id`** (`303d89a`). Simplified (dead body-types dropped, `61d0ef5`). **17.1 hardening** (`f6d96b5`): session expiry **idle 8h + absolute 24h** + login lockout **5 fails → 15-min per-username** (both were wrongly deferred; now required + built, 12/12 throwaway-DB checks). `import app` OK (47 routes, 9 auth/users endpoints), FE `npm run build` green (71 mods), **tracker.db untouched**. **Live UI login walk = Omer.**
...
> **Built + verified.** 18A/18B/18C cherry-picked clean (`6f1123e`,`a336872`,`d843672`, disjoint). 2 same-type consultants: **18A backend no FIX-NOW** (every mutation admin-gated, every by-id read team-scoped, list-filtering correct, 404-before-403, provision-on-team-create wired). **FE 2 FIX-NOW fixed** (`aff3afe`): gated the shared `ArtifactDetailModal` "Edit report" link; guarded the ungated `/domains/extract` create page. Simplified (`dc9c960`). `import app` OK (47 routes), FE build green, tracker.db untouched. **RESOLVED (Omer chose read-only-for-all) → Wave 18.1** (`48aa038`): report editor gets a `readOnly` mode (every control static + `<fieldset disabled>` backstop; create/preview stay admin-only; report links show View/Edit by role) — code-reviewed **clean**, also closed a pre-existing admin-older-report hole. **Login page redesigned** (`7de3d5f`): centered card+button, prominent title, readable error, dark-mode tokens. **Live UI walk = Omer.**
